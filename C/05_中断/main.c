...
void Int0Init(void);              // 初始化中断0函数
void LedLattice(u8 led[], u16 t); // led点阵显示函数,8*8的led点阵
...
void Int0Init(void)
{
    IT0 = 1; // P3.2可由IT0(TCON.0)选择其为低电平有效还是下降沿有效。当CPU检测到P3.2引脚上出现有效的中断信号时，中断标志IE0(TCON.1)置1，向CPU申请中断。
    EX0 = 1; // 外部中断0允许位；和IT0一样在TCON寄存器上
    EA = 1;  // CPU开中断位1
}
